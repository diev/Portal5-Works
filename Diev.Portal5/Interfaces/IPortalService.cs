...
public interface IPortalService
{
    ...
    Task<ApiResult<bool>> DownloadMessageFilesAsync(Message message, string path);
    Task<ApiResult<bool>> DownloadMessageJsonAsync(string msgId, string path, bool overwrite = false);
    Task<ApiResult<bool>> DownloadMessageZipAsync(string msgId, string path);
    ...
    Task<ApiResult<List<Message>>> GetMessagesAsync(MessagesFilter filter);
    string GetMessageUrl(string msgId);
    string GetTempName();
    string GetTempPath(string path);
    string MakeUrl(string path);
    Task<ApiResult<bool>> SaveMessageJsonAsync(Message message, string path);
    Task<ApiResult<bool>> SaveMessagesJsonAsync(List<Message> messages, string path);
    Task<ApiResult<bool>> SignAndEncryptToDirectoryAsync(string path, List<string> encryptTo, string store);
