...
internal class Zadacha221(
    ILogger<Zadacha221> logger,
    IOptions<CryptoSettings> crypto,
    IOptions<Zadacha221Settings> options,
    IApiService api,
    ...
    ) : IZadacha221
{
    ...
    {
        if (crypto.Value.EncryptTo is null)
            throw new TaskException("В конфиге не указано на кого шифровать");

        var upload = new DirectoryInfo(options.Value.UploadPath);
        ...
        if (guid == Guid.Empty)
        {
            ...
            {
                //string encFile =
                    await MakeReply2Async(files);
            }
        }
        else
        {
            Guid repId = (Guid)guid;
            string id = repId.ToString();
            ...
            foreach (var xmlFile in upload.GetFiles("PB1_ZBR_*.xml"))
            {
                string encFile = await MakeReply1Async(xmlFile);
                if (await SendReplyAsync(repId, encFile))
                    File.Delete(encFile);
            }
            ...
            if (files.Length > 0)
            {
                string encFile = await MakeReply2Async(files);
                if (await SendReplyAsync(repId, encFile))
                    File.Delete(encFile);
            }
    ...
    private async Task<string> MakeReply1Async(FileInfo xmlFile)
    {
        logger.LogTrace("Подпись и шифрование");

        string uploadPath = options.Value.UploadPath;
        string zip = options.Value.Zip;
        ...
        if (!await cryptor.SignDetachedFileAsync(zipFile, sigFile))
        ...
        if (!await cryptor.EncryptFileAsync(tptFile, encFile, crypto.Value.EncryptTo))
            throw new TaskException(
        ...
        {
            // Backup
            string archivePath = options.Value.ArchivePath;
        ...
        {
            logger.LogWarning("Переместить в архив и очистить папку отправки не удалось");
        }

        return encFile;
    }

    private async Task<string> MakeReply2Async(FileInfo[] files)
    {
        logger.LogTrace("Подпись и шифрование");
    ...
    private async Task<bool> SendReplyAsync(Guid guid, string encFile)
    {
        logger.LogTrace("Отправка файла");
        string msgId = await UploadAsync(encFile, guid);
        Thread.Sleep(60000);

        logger.LogTrace("Запрос статуса принятия");
        var result = await portal.CheckStatusAsync(msgId, 20);

        if (result.OK)
        {
            var message = result.Data;
            string report = $"Файл {encFile.PathQuoted()}, статус '{message?.Status}'.{Environment.NewLine}{_title}";
            await notifier.DoneAsync(null, report, options.Value.Subscribers);
            return true;
        }

        await notifier.FailAsync(null, result.Error!.ErrorMessage);
        return false;
    }
    ...
    private async Task<string> UploadAsync(string path, Guid guid)
    {
        var msgIdResult = await portal.UploadEncFileAsync(_task, _title, path, guid);
