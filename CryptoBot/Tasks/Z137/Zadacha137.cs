...
internal class Zadacha137(
    ILogger<Zadacha137> logger,
    IOptions<CryptoSettings> crypto,
    IOptions<Zadacha137Settings> options,
    ...
    ) : IZadacha137
{
    ...
    public async Task<int> RunAsync(uint? days)
    {
        ...
        string temp = portal.GetTempPath(uploadPath);

#if NET10_0_OR_GREATER
        await ZipFile.ExtractToDirectoryAsync(zipFullName, temp, true);
#else
        await AsyncZipFile.ExtractToDirectoryAsync(zipFullName, temp);
#endif

        logger.LogTrace("Проверка XML по схеме XSD");
        ...
        await portal.SignAndEncryptToDirectoryAsync(zipFullName, crypto.Value.EncryptTo, temp);

        logger.LogTrace("Отправка файла");

        string msgId = await UploadAsync(temp);

        logger.LogTrace("Ожидание регистрации");
        ...
        if (result.OK)
        {
            ...
            var file = zipFile.PathQuoted();

            logger.LogInformation("Файл {File}, статус '{Status}'", file, message.Status);
            string report = $"Файл {file}, статус '{message.Status}'.{Environment.NewLine}{_title}";

            return await notifier.DoneAsync(null, report, options.Value.Subscribers);
        }

        return await notifier.FailAsync(null, "Судьба отправки неизвестна", options.Value.Subscribers);
    }
    ...
    private async Task<string> UploadAsync(string path)
    {
        ...
        var msgIdResult = await portal.UploadDirectoryAsync(_task, _title, path);
